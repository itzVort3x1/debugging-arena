...
function StatTile({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-lg border border-vscode-border bg-vscode-bg-elevated/60 p-4">
      ...
      <div className="mt-1 text-lg font-semibold text-vscode-fg">{value}</div>
    </div>
  );
}
      ...
      <div className="relative mx-auto max-w-3xl px-6 py-16">
        ...
        <div className="mt-8 flex flex-col items-center text-center">
          ...
          >
            ...
            <span className="text-sm text-vscode-fg-subtle">/ 100</span>
          </div>
        </div>
        ...
        <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
          <StatTile label="Time" value={formatDuration(session.timeTaken)} />
          <StatTile label="Test runs" value={String(session.attemptsCount)} />
          <StatTile label="Hints" value={String(session.revealedHintLevels.length)} />
          <StatTile label="Tests" value={total > 0 ? `${passed}/${total}` : "—"} />
        </div>
