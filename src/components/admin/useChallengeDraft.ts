...
export function useChallengeDraft({
    ...
    const addFile = useCallback((path: string) => {
        setTree((prev) =>
            prev[path] !== undefined ? prev : { ...prev, [path]: "" },
        );
        setActivePath(path);
    }, []);
    ...
    const deleteFile = useCallback(
        (path: string) => {
            setTree((prev) => {
                ...
                delete next[path];
                if (path === activePath)
                    setActivePath(sortedPaths(next)[0] ?? "");
                return next;
            });
        },
        [activePath],
    ...
    const validate = useCallback(async () => {
        ...
        try {
            ...
            if (res.ok)
                setNotice("Valid - this challenge would publish cleanly.");
        } catch (err) {
        ...
        async (nextStatus: ChallengeStatus) => {
            ...
            try {
                const res = await apiFetch<SaveResponse>(
                    ...
                    {
                        method: "PUT",
                        json: { content: tree, status: nextStatus },
                    },
                );
                ...
                const detail = (body as { errors?: string[] } | undefined)
                    ?.errors;
