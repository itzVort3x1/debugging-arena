      ...
      {groups.map(([label, paths]) =>
        paths.length === 0 ? null : (
          <div key={label} className="text-[11px]">
            ...
            <span className="text-vscode-fg-subtle">{paths.join(", ")}</span>
          </div>
        ),
      )}
    </div>
  ...
  return (
    <aside className="flex w-80 shrink-0 flex-col border-l border-vscode-border bg-vscode-sidebar">
      ...
      <div className="flex-1 overflow-y-auto px-3 pb-3">
        {error ? (
          <p className="mb-2 text-[11px] text-vscode-error">{error}</p>
        ) : null}
              ...
              return (
                <li
                  ...
                  className={cn(
                    ...
                    isOpen && "border-vscode-border bg-vscode-bg-elevated",
                  )}
                >
                  <button
                    type="button"
                  ...
                  >
                    <Badge size="sm" tone={isOpen ? "accent" : "neutral"}>
                      v{v.version}
                    </Badge>
                    ...
                    {loadingVersion === v.version ? <Spinner size="sm" /> : null}
                  </button>
                  ...
                  {isOpen && selected ? (
                    <div className="space-y-2 border-t border-vscode-border-subtle px-2 py-2">
                      ...
                      <Button
                        ...
                        variant="secondary"
                        className="w-full"
                        disabled={treesMatch(currentTree, selected.content)}
                        onClick={() =>
                          onLoad(
                            selected.content,
